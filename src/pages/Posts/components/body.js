        ...
        <>
            <form className="flex p-5 gap-5 flex-col mb-5">
                <div className='flex flex-row gap-5 justify-between'>
                    <label htmlFor="rap" className='text-lg'>Rạp đang công chiếu</label>
                ...
                <div className='flex flex-row gap-5 justify-between'>
                    <label htmlFor="ngaychieu" className='text-lg'>Chọn ngày xem</label>
    ...
    return (
        <>
            <div className=' md:container md:mx-auto mt-5 mx-2 md:mt-0'>
                ...
                <div className='mx-2 mt-4 p-2 border-x-4 text-justify md:text-start border-pink-500/50'>
                    <p className='text-xl font-bold font-mono'>Nội dung</p>
                    {Data ? decode(Data.description).replace(/(\r\n|\n|\r)/gm, "").match(/<p.*?>(.*?)<\/p>/g).map((v, i) => {
                        v = v.replace(/(<([^>]+)>)/g, "")
                        return v.length > 5 ? (
